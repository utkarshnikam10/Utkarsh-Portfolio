...
export function OrganicCore({ visible }: OrganicCoreProps) {
  ...
  const handleBeforeCompile = (
    ...
    shader.vertexShader = shader.vertexShader.replace(
      ...
      `
      #include <begin_vertex>
      float noiseVal = snoise(position * 1.8 + vec3(uTime * 0.7)) * (0.05 + uRipple * 0.15);
  ...
  useFrame((state, delta) => {
    ...
    if (visible) {
      ...
      uniformsRef.current.uRipple.value = physics.speed * 0.5;

      meshRef.current.rotation.y += delta * (0.2 + physics.speed * 0.1);
    }
  });

  return (
    <Float speed={1.5} rotationIntensity={0.3} floatIntensity={0.5}>
      <mesh ref={meshRef} position={[0, 0, 0]} material={glassMaterial}>
